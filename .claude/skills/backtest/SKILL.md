...
### 3. 가설 게이트 (기본 3회 · 조건부 5회 — PROTOCOL §4-(a))
Agent 도구(`model: opus`)로 독립 검토를 돌린다. 프롬프트에 명시할 것:
- `docs/analysis/PROTOCOL.md` §4-(a) 체크리스트 **6개 항목**으로 검토하라
- `registry.md` 전체·`jail.md`와 직접 대조하라 (파일 경로 전달)
- **항목별로 번호를 붙여 통과/미통과를 반환하라** (연장 여부 판단에 필요)
- 반환: 통과/미통과 + 항목별 사유

미통과 → 사유 반영해 가설을 수정하고 재검토.

- **매 회차 1·2·3(중복·구현가능·조건부합)을 전부 통과**했고 4·5·6에서만 걸렸다면
  **최대 5회까지 연장**한다. 그 외에는 3회에서 종료.
- 연장 중이라도 ① 1·2·3 중 하나가 새로 걸리거나 ② 같은 지적이 2회 연속 반복되거나
  ③ 수정이 새 결함을 만드는 일이 2회 연속이면 **즉시 기각**.
- **수식·산식을 문서에 적었다면 검증(극단값·차원·소형 예제)을 함께 남긴다** — 게이트
  단계엔 코드가 없어 수식 오류가 그대로 통과한다(항목 6).

기각 시 registry 결과 요약에 **"게이트 미통과(테스트 미실행)"**과 최종 사유를 명기해
테스트 후 기각과 구분하고, **설계 문서는 지우지 않는다**(재시도 출발점).
1번으로 돌아가 다음 가설을 선택한다.
(이 횟수는 `/backtest-refresh`의 수혈 시도 3회와 별개 카운터다.)
